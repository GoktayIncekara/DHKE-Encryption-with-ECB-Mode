...
#For example: >>python3 alice.py dhke -a 4 -B 10 -p 23
...
def main():
    ...
    if (sys.argv[1]=="dhke"):
        if(len(sys.argv)==6):
            ...
            a = randomNumber()  #generates private key of Alice as a random number 
            A= (g**a)%p  #computes the public key of Alice
            print("a = ", a, BOLD + " (This must be kept secret.)" + END_COLOR)
            print("A = ", A, BOLD + " (This can be sent to Bob.)" + END_COLOR)

        elif (len(sys.argv)==8):
            if(sys.argv[2] == "-a" and sys.argv[4]=="-B" and sys.argv[6]=="-p"):
                a = sys.argv[3]
                B = sys.argv[5]
                p = sys.argv[7]
                ...
                p = sys.argv[5]
                a = sys.argv[7]
            elif(sys.argv[2] == "-B" and sys.argv[4]=="-a" and sys.argv[6]=="-p"):
                B = sys.argv[3]
                a = sys.argv[5]
                p = sys.argv[7]
            else:
                raise Exception("Please provide a, p and B to calculate the key.")
        ...
        if(sys.argv[2] == "-p" and sys.argv[4]=="-k"):
            p = sys.argv[3]
            k = sys.argv[5]
        elif (sys.argv[2] == "-k" and sys.argv[4]=="-p"):
            k = sys.argv[3]
            p = sys.argv[5]
        else:
            raise Exception("Please provide the plaintext and a key.")    
        ...
        encodedPlain = bytes(p, 'ascii')  #converts string to byte
        trailing_plain=trailingSpaces(encodedPlain)  #adds space at the end of the plain text if needed
        
        cipher = DES.new(key, DES.MODE_ECB)  
        msg = cipher.encrypt(pad(trailing_plain, BLOCK_SIZE)) #encrypt des function with using ECB mode
        print(START_CYAN+'Raw ciphertext (Normally this is sent to Bob over a network):\n'+END_COLOR, msg)
        cip_dec = base64.b64encode(msg) #encode raw ciphertest to readable ciphertext
        readable_cip= cip_dec.decode()
        print(START_CYAN+'Readable ciphertext (For the sake of simplicity, we will send this to Bob. This can also be used if we use pen and paper to deliver the message.):\n'+ END_COLOR, readable_cip)
